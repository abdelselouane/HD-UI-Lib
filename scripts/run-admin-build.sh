...
echo "***[run-build.sh]***: Installing Repair-Parts-Admin Dependencies"
cd packages/Repair-Parts-Admin 
npm i

echo "***[run-build.sh]***: Build Repair-Parts-Admin Package"
npm run build
...
echo "*** [run-build.sh] ***: Publish Repair-Parts-Admin Package to npm Artifactory"
